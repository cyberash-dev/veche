...
- **`/veche` skill per-turn timeout raised 120s → 300s, polling budget
  10m → 15m.** Committee turns on heavy dev questions routinely exceed
  120s (a single thorough Claude/Codex answer can run several minutes), so
  members were dropped with `AdapterTurnTimeout` before they could reply.
  The larger budgets let realistic turns complete.
