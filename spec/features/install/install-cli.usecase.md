...
7. For each target, in declaration order, do:
   - 7a. **Skill files** (skipped if `--mcp-only`):
...
| `HostCliMissing` | `claude` / `codex` could not be resolved (PATH miss, PATHEXT-aware on Windows) and `--force` was NOT supplied. | `2` | stderr |
| `HostCliSpawnFailed` | Host CLI was resolved (path printed) but launching it failed (e.g. broken Windows wrapper, non-executable file). | `2` | stderr |
...
- **Bounded write surface.** Filesystem writes go ONLY to (a) `<host-skills-root>/<mcp-name>/SKILL.md`, (b) `<host-skills-root>/<mcp-name>/agents/openai.yaml` when present in the package, and (c) `$VECHE_HOME/config.json`. A test verifies that no `fs.writeFile` / `fs.rename` happens outside those paths.
- **Atomic writes.** `SKILL.md`, optional `agents/openai.yaml`, and `config.json` are written via `<path>.tmp-<pid>-<ts>` then `rename`. Mirrors the existing `show --out` pattern. Readers (Claude Code / Codex / `loadConfig`) never see a half-written file.
...
- **Override env vars are honoured.** `CLAUDE_BIN` and `CODEX_BIN` are read from the environment and used when set, mirroring the same vars used by [agent-integration](../agent-integration/agent-integration.md) when spawning members. They may be a bare command name OR an absolute path (including a Windows `.cmd` / `.bat` wrapper such as `C:\Users\<u>\AppData\Roaming\npm\codex.cmd`). The installer resolves the override using PATHEXT semantics on `process.platform === 'win32'`. The install command does not introduce new env vars.
- **Windows wrapper launch.** When the resolved host CLI is a `.cmd` or `.bat` file on Windows, the installer launches it via `cmd.exe /d /s /c <resolved-bin> <args...>` (Node's documented safe form for batch wrappers post CVE-2024-27980). Each argument is individually quoted using the standard Windows argv rule. On non-Windows or for plain executables, the installer spawns the resolved path directly with no shell. The argv that the host CLI sees is identical across platforms.
